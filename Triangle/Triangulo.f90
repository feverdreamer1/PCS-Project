program Ising_Triangular
        use randomnumber
        implicit none

        integer :: pMC_total 
        integer, parameter :: pMC_medida = 100    
        integer :: num_medidas 

        real*8, parameter :: J = 1.0d0 
        ...
        T_vals = (/ 2.00d0, 2.50d0, 3.00d0, 3.30d0, 3.40d0, 3.50d0, 3.55d0, 3.58d0, 3.60d0, 3.62d0, &
                    3.63d0, 3.64d0, 3.65d0, 3.66d0, 3.68d0, 3.70d0, 3.75d0, 3.80d0, 3.90d0, 4.00d0, &
                    4.20d0, 4.50d0, 5.00d0, 5.50d0, 6.00d0 /)       
                    
        open(10, file="termodinamica_triangular.dat", status="replace")
        write(10, *) "# N      T         m_N            err_M          e_N            err_E          c_N            err_C"
        
        open(20, file="correlacion_triangular.dat", status="replace")
        write(20, *) "# T      Dist(r)   f(r)           err_f (Solo para N=512)"
        ...
        do in_val = 1, 6
            ...
            if (N >= 256) then
                pMC_total = 550000
                write(*,*) ">> Iniciando red N =", N, "(Modo Rápido: 550k pMC)"
            else
                pMC_total = 1000000
            ...
            do it_val = 1, 25
                ...
                e_N = avg_E / dble(3*N*N) 
                err_E = sqrt(abs(avg_E2 - avg_E**2) / dble(num_medidas)) / dble(3*N*N)
    ...
    contains
        ...
        subroutine paso_montecarlo(red, N_dim, Temp)
            ...
            do intento = 1, N_dim * N_dim
                ...
                sum_vecinos = dble(red(arriba, c) + red(abajo, c) + red(f, izq) + &
                                   red(f, dcha) + red(arriba, izq) + red(abajo, dcha))

                deltae = 2.0d0 * dble(red(f, c)) * J * sum_vecinos
        ...
        subroutine medir_energia_y_mag(red, N_dim, E_tot, M_tot)
            ...
            integer :: i_sub, j_sub, dcha_sub, abajo_sub
            real*8 :: E_enlaces
            ...
            do i_sub = 1, N_dim
                do j_sub = 1, N_dim
                    ...
                    E_enlaces = dble(red(i_sub, dcha_sub) + red(abajo_sub, j_sub) + red(abajo_sub, dcha_sub))
                    
                    E_tot = E_tot - dble(red(i_sub, j_sub)) * J * E_enlaces
                end do
            end do
